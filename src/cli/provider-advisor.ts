...
function resolveProviderBinary(provider: ProviderAdvisorName, env: NodeJS.ProcessEnv): string {
  const override = env[`${PROVIDER_BINARY_ENV_PREFIX}${provider.toUpperCase()}_BIN`]?.trim();
  return override || PROVIDER_BINARIES[provider];
}

function resolveProviderScript(provider: ProviderAdvisorName, env: NodeJS.ProcessEnv): string | null {
...
export async function executeProviderAdvisor(
  ...
  const exitCode = run.errorMessage && run.exitCode === 0 ? 1 : run.exitCode;
