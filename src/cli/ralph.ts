...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- Identify implementation, evidence/regression, and final sign-off lanes before delegating.',
    '- When 2 or more lanes are independent, launch the full subagent batch first and only then begin waiting/joining work.',
    '- Do not serialize independent subagent launches by waiting on one lane before starting another.',
    '- Use Codex native subagents for bounded in-session fanout; use `omx team` when the work needs durable tmux workers, worktrees, or long-running coordination.',
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_policy: 'Independent Ralph lanes should launch as one native-subagent batch when possible, and phase completion must wait for active native subagent threads to finish.',
