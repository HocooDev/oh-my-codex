...
- In consensus mode, the Planner launches Architect and Critic against the same draft in parallel, then explicitly reconciles conflicting feedback in the next revision.
...
- Use the surface-appropriate structured question path only for real preferences/branches (`omx question` in attached tmux, native structured input when available, plain text only as last fallback).
- Use Write to save plans.
- In consensus mode, batch Architect and Critic review requests together and wait for both before revising the draft.
